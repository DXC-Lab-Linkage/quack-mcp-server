    ...
    async def process(self, job: LintJob) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"[{job.job_type.value}:{job.id}] Running pylint on {temp_path}"
                )
...
""".encode("utf-8")
                ...
                # Run the script
                logger.debug(
                    f"[{job.job_type.value}:{job.id}] Running script: {script_path}"
                )
                ...
                logger.debug(
                    f"[{job.job_type.value}:{job.id}] Script stdout: {result.stdout}"
                )
                logger.debug(
                    f"[{job.job_type.value}:{job.id}] Script stderr: {result.stderr}"
                )
                logger.debug(
                    f"[{job.job_type.value}:{job.id}] Script return code: {result.returncode}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"[{job.job_type.value}:{job.id}] Failed to clean up script file: {str(e)}"
                    )
                ...
                if result.returncode != 0:
                    ...
                    logger.error(
                        f"[{job.job_type.value}:{job.id}] Script failed: {error_msg}"
                    )
                ...
                logger.debug(
                    f"[{job.job_type.value}:{job.id}] Parsing lint output: {lint_output}"
                )
                ...
                try:
                    lint_results = (
                        json.loads(lint_output) if lint_output.strip() else []
                    )
                    logger.debug(
                        f"[{job.job_type.value}:{job.id}] Parsed lint results: {lint_results}"
                    )
                except json.JSONDecodeError as e:
                    logger.error(
                        f"[{job.job_type.value}:{job.id}] Failed to parse JSON: {str(e)}"
                    )
                    job.status = JobStatus.FAILED
                ...
                # Create result
                issue_count = (
                    len(errors) + len(warnings) + len(conventions) + len(refactors)
                )
                job.result = {
                    "status": "success",
                    "summary": {
                ...
                logger.error(
                    f"[{job.job_type.value}:{job.id}] Error running pylint: {str(e)}",
                    exc_info=True,
                )
                job.status = JobStatus.FAILED
