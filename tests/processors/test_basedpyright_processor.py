...
@pytest.mark.asyncio
async def test_basedpyright_processor_success(basedpyright_processor, sample_basedpyright_job):
...
@pytest.mark.asyncio
async def test_basedpyright_processor_stderr_error(basedpyright_processor, sample_basedpyright_job):
    """Test basedpyright processor when stderr contains errors"""
    mock_process = AsyncMock()
    mock_process.communicate.return_value = (b"", b"basedpyright: command not found")
    mock_process.pid = 12345
...
@pytest.mark.asyncio
async def test_basedpyright_processor_timeout(basedpyright_processor, sample_basedpyright_job):
...
@pytest.mark.asyncio 
async def test_basedpyright_processor_invalid_json(basedpyright_processor, sample_basedpyright_job):
...
@pytest.mark.asyncio
async def test_basedpyright_processor_retry_logic(basedpyright_processor, sample_basedpyright_job):
    ...
    side_effects = [OSError("Connection failed"), OSError("Connection failed"), mock_process]
...
@pytest.mark.asyncio
async def test_basedpyright_processor_retry_exhausted(basedpyright_processor, sample_basedpyright_job):
    """Test basedpyright processor when all retry attempts fail"""
    with patch("asyncio.create_subprocess_exec", side_effect=OSError("Persistent error")):
        with patch("asyncio.sleep"):  # Speed up the test by mocking sleep
            await basedpyright_processor.process(sample_basedpyright_job)
